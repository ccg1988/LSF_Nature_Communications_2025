...
figure('position',[X_size*0.02 Y_size*0.35 X_size*0.3 Y_size*0.5]);%1 line
rng('default') % fix the random number generator (rng)
range = 100 ; %line plot range
smr = 3 ;%line smooth range
spkr_rank=find(TSpkr_Rank>=13);
...
peak_times = zeros(N_units, 1);
for i = 1: N_units
    trace = smooth(zsU(i, :), smr); % 3&4 the same
    trace = trace/max(trace);
    smoothed_data(i, :) = trace;
    [~, peak_times(i)] = max(smoothed_data(i, :)); 
end
[sorted_peak_times, sort_indices] = sort(peak_times); % Sort variables based on the time of the first peak
