...
Nsd = 1 ; % number of standard deviation above baseline that is consecutive 
Ns=1; % number of shuffle for each neuron; previously used 1000times 
consec_len=19; % smaller STD could have more consecutive trials
trial_len_U=50; % lower limit for considering consec-trials in UFile
trial_len_E=10; % lower limit for computing mu&sigma in EFile
N_void=[];
...
if Nsd==1.5 %only occurs in this rng and abnormal
% consecN_adap_real(584,14:19)=0;
end
%% plot the figures
figure;
...
F=[]; A=[]; p_faci=nan(19,1); p_adap=nan(19,1);
for i = 1:19
...
if Nsd==1
p_faci(13:end)=[]; p_adap(13:end)=[];
end
if Nsd==1.5
p_faci(11:end)=[]; p_adap(11:end)=[];
end
if Nsd==2
p_faci(9:end)=[]; p_adap(9:end)=[];
end
